  ...
  <div>
    <h3 class="mt-3 mb-3">商品列表</h3>
     <el-table :data="items">
          ...
          <el-table-column prop="dizhi" label="产地" ></el-table-column>
          <el-table-column prop="lianxi" label="联系方式" ></el-table-column>
          <el-table-column fixed="right" label="操作" width="180">
      <template slot-scope="scope">
        <el-button type="primary" size="small"
         @click="$router.push(`/botanies/edit/${scope.row._id}`)">编辑</el-button>
         <el-button type="primary" size="small"
...
export default {
  ...
  methods:{
   async fetch(){
    const res = await this.$http.get('rest/botanies')
    this.items = res.data
    },
    async remove(row){
      this.$confirm(`是否要确认删除分类?"${row.name}"`, '提示', {
        ...
        .then(async() => {
          const res=await this.$http.delete(`rest/botanies/${row._id}`)
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
